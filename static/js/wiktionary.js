...
const Dictionary = {
  ...
  longest(text) {
      ...
      .filter((word) => {
        ...
        if (first) {
          ...
          if (matchedIndex !== -1 && !this.isCombining(word.search.charAt(0))) {
            ...
            // console.log(`nextChar "${nextChar}" is not combining`)
            first = word.head
    ...
    // console.log('😄 final matched text: ' + matchedText, matches)
    return {
      matches: matches,
      text: matchedText
    }
