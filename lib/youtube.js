...
export default {
  ...
  async getRandomEpisodeYouTubeId(langId, type) {
    ...
    let url = `${Config.wiki}items/youtube_videos?filter[l2][eq]=${langId}${showFilter}&filter[youtube_id][contains]=${randBase64Char}&fields=youtube_id`
  ...
  async searchWithLike({ terms, langId, filter, adminMode, excludeTerms, continua, limit = 20, limitToTVShow = false, exact = false, apostrophe = false, convertToSimplified = false } = {}) {
    let showFilter = ''
    ...
    for (let term of terms) {
      ...
      try {
        ...
        let url = `${Config.wiki}items/youtube_videos?filter[l2][eq]=${langId
          }${showFilter}&${subsFilter}${limitQuery}&timestamp=${adminMode ? Date.now() : 0
          }`
  ...
  async searchSubs({terms, excludeTerms, lang = 'en', langId = 1824, adminMode = false, continua = true, limit = 20, limitToTVShow = false, exact = false, apostrophe = false, convertToSimplified = false} = {}) {
    let hits = []
    hits = hits.concat(await this.searchWithLike({ terms, langId, filter: 'tv_show', adminMode, excludeTerms, continua, limit, limitToTVShow, exact, apostrophe, convertToSimplified }))
    if ((!limit || hits.length < limit) && !limitToTVShow && hits.length < 10) {
      hits = hits.concat(await this.searchWithLike({ terms, langId, filter: 'talk', adminMode, excludeTerms, continua, limit, limitToTVShow, exact, apostrophe, convertToSimplified }))
    }
    if ((!limit || hits.length < limit) && ![].includes(lang) && !limitToTVShow && hits.length < 10) {
      hits = hits.concat(await this.searchWithLike({ terms, langId, filter: false, adminMode, excludeTerms, continua, limit, limitToTVShow, exact, apostrophe, convertToSimplified }))
    }
    ...
    for (let video of videos) {
      ...
      let keptIndexes = video.subs_l2.map((l, i) => i).filter(i => {
        for (let index of matchedLineIndexes) {
          if (i > index - 5 && i < index + 5) return true
        }
      })
